...
public class CortexBaker extends Upgrade {

    public CortexBaker(CookieTycoon plugin) {
        super(plugin);
    }
    ...
    public String getName() {
        return ChatColor.BOLD + "Мозговой пекарь";
    }

    @Override
    public String getLore() {
        return ChatColor.YELLOW + "Этот искусственный мозг размером с планету ";
    }

    public String getLore2() {
        return ChatColor.YELLOW + "способен воплощать мечты о печенье. Пространство и ";
    }

    public String getLore3() {
        return ChatColor.YELLOW + "время не имеют значения. Реальность произвольна.";
    }
    ...
    public double getPrice(Player player) {
        return (1.9 * Math.pow(10, 24) * Math.pow(1.15, getSum(player))); // 1,9Spt == 1.9 * 10^24 * 1,15^N
    }

    @Override
    public double getSum(Player player) {
        return plugin.getUpgradesManager().get(player, "cortexbaker");
    }

    @Override
    public double getMultiplier(Player player) {
        return (64 * Math.pow(10, 12) * plugin.getAchievementManager().getCombinedMultiplier(player, "cortexbaker") + plugin.getAchievementManager().getCombinedTerm(player, "cortexbaker"))* getSum(player); // 64T
    }
}
