...
public class Portal extends Upgrade {

    public Portal(CookieTycoon plugin) {
        super(plugin);
    }
    ...
    public String getName() {
        return ChatColor.BOLD + "Портал";
    }

    @Override
    public String getLore() {
        return ChatColor.YELLOW + "Открывает дверь в Печеньковселенную.";
    }
    ...
    public double getPrice(Player player) {
        return (Math.pow(10, 12) * Math.pow(1.15, getSum(player))); // 1T = 10^12 * 1,15^N
    }

    @Override
    public double getSum(Player player) {
        return plugin.getUpgradesManager().get(player, "portal");
    }

    @Override
    public double getMultiplier(Player player) {
        return (10_000_000 * plugin.getAchievementManager().getCombinedMultiplier(player, "portal") + plugin.getAchievementManager().getCombinedTerm(player, "portal"))* getSum(player);
    }
}
