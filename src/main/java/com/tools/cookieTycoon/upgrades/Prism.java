...
public class Prism extends Upgrade {

    public Prism(CookieTycoon plugin) {
        super(plugin);
    }
    ...
    public String getName() {
        return ChatColor.BOLD + "Призма";
    }

    @Override
    public String getLore() {
        return ChatColor.YELLOW + "Преобразует свет в печенье.";
    }
    ...
    public double getPrice(Player player) {
        return (2.1 * Math.pow(10, 15) * Math.pow(1.15, getSum(player))); // 2,1Quad == 2,1 * 10^15 * 1,15^N
    }

    @Override
    public double getSum(Player player) {
        return plugin.getUpgradesManager().get(player, "prism");
    }

    @Override
    public double getMultiplier(Player player) {
        return (2.9 * Math.pow(10, 9) * plugin.getAchievementManager().getCombinedMultiplier(player, "lab") + plugin.getAchievementManager().getCombinedTerm(player, "lab"))* getSum(player); // 2,9B
    }
}
