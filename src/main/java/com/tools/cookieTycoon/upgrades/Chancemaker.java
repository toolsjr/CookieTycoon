...
public class Chancemaker extends Upgrade {

    public Chancemaker(CookieTycoon plugin) {
        super(plugin);
    }
    ...
    public String getName() {
        return ChatColor.BOLD + "Шансмейкер";
    }

    @Override
    public String getLore() {
        return ChatColor.YELLOW + "Создает печенье из воздуха благодаря чистой удачи.";
    }
    ...
    public double getPrice(Player player) {
        return (26 * Math.pow(10, 15) * Math.pow(1.15, getSum(player))); // 26Quad == 26 * 10^15 * 1,15^N
    }

    @Override
    public double getSum(Player player) {
        return plugin.getUpgradesManager().get(player, "chancemaker");
    }

    @Override
    public double getMultiplier(Player player) {
        return (21 * Math.pow(10, 9) * plugin.getAchievementManager().getCombinedMultiplier(player, "chancemaker") + plugin.getAchievementManager().getCombinedTerm(player, "chancemaker"))* getSum(player); // 21B
    }
}
