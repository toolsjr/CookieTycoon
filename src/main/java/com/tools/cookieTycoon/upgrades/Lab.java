...
public class Lab extends Upgrade {

    public Lab(CookieTycoon plugin) {
        super(plugin);
    }
    ...
    public String getName() {
        return ChatColor.BOLD + "Алхимическая лаборатория";
    }

    @Override
    public String getLore() {
        return ChatColor.YELLOW + "Превращает золото в печенье!";
    }
    ...
    public double getPrice(Player player) {
        return (75 * Math.pow(10, 9) * Math.pow(1.15, getSum(player))); // 75B = 75 * 10^9 * 1,15^N
    }

    @Override
    public double getSum(Player player) {
        return plugin.getUpgradesManager().get(player, "lab");
    }

    @Override
    public double getMultiplier(Player player) {
        return (1_600_000 * plugin.getAchievementManager().getCombinedMultiplier(player, "lab") + plugin.getAchievementManager().getCombinedTerm(player, "lab"))* getSum(player);
    }
}
