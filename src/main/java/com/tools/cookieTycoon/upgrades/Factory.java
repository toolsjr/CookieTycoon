...
public class Factory extends Upgrade {
    ...
    public Factory(CookieTycoon plugin) {
        super(plugin);
    }
    ...
    public String getName() {
        return ChatColor.BOLD + "Фабрика";
    }

    @Override
    public String getLore() {
        return ChatColor.YELLOW + "Производит большое количество печенья.";
    }
    ...
    public double getPrice(Player player) {
        return (130000 * Math.pow(1.15, getSum(player)));
    }

    @Override
    public double getSum(Player player) {
        return plugin.getUpgradesManager().get(player, "factory");
    }

    @Override
    public double getMultiplier(Player player) {
        return (260 * plugin.getAchievementManager().getCombinedMultiplier(player, "factory") + plugin.getAchievementManager().getCombinedTerm(player, "factory")) * getSum(player);
    }
}
