...
public class Engine extends Upgrade {
    ...
    public Engine(CookieTycoon plugin) {
        super(plugin);
    }
    ...
    public String getName() {
        return ChatColor.BOLD + "Фрактальный двигатель";
    }

    @Override
    public String getLore() {
        return ChatColor.YELLOW + "Увеличивает количество печенья.";
    }
    ...
    public double getPrice(Player player) {
        return (310 * Math.pow(10, 15) * Math.pow(1.15, getSum(player))); // 26Quad == 26 * 10^15 * 1,15^N
    }

    @Override
    public double getSum(Player player) {
        return plugin.getUpgradesManager().get(player, "engine");
    }

    @Override
    public double getMultiplier(Player player) {
        return (150 * Math.pow(10, 9) * plugin.getAchievementManager().getCombinedMultiplier(player, "engine") + plugin.getAchievementManager().getCombinedTerm(player, "engine"))* getSum(player);
    }
}
