...
public class Bank extends Upgrade {
    ...
    public Bank(CookieTycoon plugin) {
        super(plugin);
    }
    ...
    public String getName() {
        return ChatColor.BOLD + "Банк";
    }

    @Override
    public String getLore() {
        return ChatColor.YELLOW + "Генерирует печенье из процентов.";
    }
    ...
    public double getPrice(Player player) {
        return (1400000 * Math.pow(1.15, getSum(player)));
    }

    @Override
    public double getSum(Player player) {
        return plugin.getUpgradesManager().get(player, "bank");
    }

    @Override
    public double getMultiplier(Player player) {
        return (1400 * plugin.getAchievementManager().getCombinedMultiplier(player, "bank") + plugin.getAchievementManager().getCombinedTerm(player, "bank")) * getSum(player);
    }
}
