...
public class Temple extends Upgrade {

    public Temple(CookieTycoon plugin) {
        super(plugin);
    }

    @Override
    public ItemStack getItem(Player player) {
        ItemStack item = new ItemStack(Material.MOSSY_COBBLESTONE_SLAB);
    ...
    public String getName() {
        return ChatColor.BOLD + "Храм";
    }

    @Override
    public String getLore() {
        return ChatColor.YELLOW + "Заполнен драгоценным древним шоколадом.";
    }
    ...
    public double getPrice(Player player) {
        return (20_000_000 * Math.pow(1.15, getSum(player)));
    }

    @Override
    public double getSum(Player player) {
        return plugin.getUpgradesManager().get(player, "temple");
    }

    @Override
    public double getMultiplier(Player player) {
        return (7800 * plugin.getAchievementManager().getCombinedMultiplier(player, "temple") + plugin.getAchievementManager().getCombinedTerm(player, "temple")) * getSum(player);
    }
}
