...
public class Farm extends Upgrade {

    public Farm(CookieTycoon plugin) {
        super(plugin);
    }
    ...
    public String getName() {
        return ChatColor.BOLD + "Ферма";
    }

    @Override
    public String getLore() {
        return ChatColor.YELLOW + "Выращивает печеньковые растения из печеньковых семян.";
    }
    ...
    public double getPrice(Player player) {
        return (1100 * Math.pow(1.15, getSum(player)));
    }

    @Override
    public double getSum(Player player) {
        return plugin.getUpgradesManager().get(player, "farm");
    }

    @Override
    public double getMultiplier(Player player) {
        return (8 * plugin.getAchievementManager().getCombinedMultiplier(player, "farm") + plugin.getAchievementManager().getCombinedTerm(player, "farm"))* getSum(player);
    }
}
