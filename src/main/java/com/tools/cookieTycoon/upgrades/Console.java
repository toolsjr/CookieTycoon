...
public class Console extends Upgrade {

    public Console(CookieTycoon plugin) {
        super(plugin);
    }
    ...
    public ItemStack getItem(Player player) {
        ...
        meta.setLore(Arrays.asList(getLore(), getLore2(), "", "§7(ЛКМ) Купить x1 - " + NumberFormatter.format(getPrice(player)),
    ...
    public String getName() {
        return ChatColor.BOLD + "Консоль Java";
    }

    @Override
    public String getLore() {
        return ChatColor.YELLOW + "Создает печенье из того самого кода,";
    }

    public String getLore2() {
        return ChatColor.YELLOW + "на котором написана эта игра.";
    }
    ...
    public double getPrice(Player player) {
        return (71 * Math.pow(10, 18) * Math.pow(1.15, getSum(player))); // 71Qi == 71 * 10^18 * 1,15^N
    }

    @Override
    public double getSum(Player player) {
        return plugin.getUpgradesManager().get(player, "console");
    }

    @Override
    public double getMultiplier(Player player) {
        return (1.1 * Math.pow(10, 12) * plugin.getAchievementManager().getCombinedMultiplier(player, "console") + plugin.getAchievementManager().getCombinedTerm(player, "console"))* getSum(player); // 1,1T
    }
}
