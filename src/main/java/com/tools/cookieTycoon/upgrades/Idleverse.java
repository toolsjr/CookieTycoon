...
public class Idleverse extends Upgrade {

    public Idleverse(CookieTycoon plugin) {
        super(plugin);
    }
    ...
    public String getName() {
        return ChatColor.BOLD + "Пустая вселенная";
    }

    @Override
    public String getLore() {
        return ChatColor.YELLOW + "Кроме нашей существует ещё бессчетное множество ";
    }

    public String getLore2() {
        return ChatColor.YELLOW + "пустых вселенных. Вы наконец-то нашли способ украсть их ";
    }

    public String getLore3() {
        return ChatColor.YELLOW + "продукцию и превратить всё, что они сделали, в печенье!";
    }
    ...
    public double getPrice(Player player) {
        return (12 * Math.pow(10, 21) * Math.pow(1.15, getSum(player))); // 12Sx == 12 * 10^21 * 1,15^N
    }

    @Override
    public double getSum(Player player) {
        return plugin.getUpgradesManager().get(player, "idleverse");
    }

    @Override
    public double getMultiplier(Player player) {
        return (8.3 * Math.pow(10, 12) * plugin.getAchievementManager().getCombinedMultiplier(player, "idleverse") + plugin.getAchievementManager().getCombinedTerm(player, "idleverse"))* getSum(player); // 8,3T
    }
}
