...
public class Granny extends Upgrade {

    public Granny(CookieTycoon plugin) {
        super(plugin);
    }
    ...
    public String getName() {
        return ChatColor.BOLD + "Бабуля";
    }

    @Override
    public String getLore() {
        return ChatColor.YELLOW + "Хорошая бабуля испекла ещё печенья.";
    }
    ...
    public double getPrice(Player player) {
        return (100 * Math.pow(1.15, getSum(player)));
    }

    @Override
    public double getSum(Player player) {
        return plugin.getUpgradesManager().get(player, "granny");
    }

    @Override
    public double getMultiplier(Player player) {
        return (1 * plugin.getAchievementManager().getCombinedMultiplier(player, "granny") + plugin.getAchievementManager().getCombinedTerm(player, "granny"))* getSum(player);
    }
}
