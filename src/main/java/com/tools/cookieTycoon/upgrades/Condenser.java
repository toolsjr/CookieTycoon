...
public class Condenser extends Upgrade {

    public Condenser(CookieTycoon plugin) {
        super(plugin);
    }
    ...
    public String getName() {
        return ChatColor.BOLD + "Конденсатор антиматерии";
    }

    @Override
    public String getLore() {
        return ChatColor.YELLOW + "Конденсирует антиматерию Вселенной в печенье.";
    }
    ...
    public double getPrice(Player player) {
        return (170 * Math.pow(10, 12) * Math.pow(1.15, getSum(player)));
    }

    @Override
    public double getSum(Player player) {
        return plugin.getUpgradesManager().get(player, "condenser");
    }

    @Override
    public double getMultiplier(Player player) {
        return (430_000_000 * plugin.getAchievementManager().getCombinedMultiplier(player, "condenser") + plugin.getAchievementManager().getCombinedTerm(player, "condenser"))* getSum(player);
    }
}
