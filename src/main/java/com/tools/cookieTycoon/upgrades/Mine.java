...
public class Mine extends Upgrade {

    public Mine(CookieTycoon plugin) {
        super(plugin);
    }
    ...
    public String getName() {
        return ChatColor.BOLD + "Шахта";
    }

    @Override
    public String getLore() {
        return ChatColor.YELLOW + "Добывает тесто для печенья и шоколадную крошку.";
    }
    ...
    public double getPrice(Player player) {
        return (12000 * Math.pow(1.15, getSum(player)));
    }

    @Override
    public double getSum(Player player) {
        return plugin.getUpgradesManager().get(player, "mine");
    }

    @Override
    public double getMultiplier(Player player) {
        return (47 * plugin.getAchievementManager().getCombinedMultiplier(player, "mine") + plugin.getAchievementManager().getCombinedTerm(player, "mine"))* getSum(player);
    }
}