...
public class Cursor extends Upgrade {
    ...
    public Cursor(CookieTycoon plugin) {
        super(plugin);
    }
    ...
    public String getName() {
        return ChatColor.BOLD + "Курсор";
    }

    @Override
    public String getLore() {
        return ChatColor.YELLOW + "Автоматический клик каждые 10 секунд.";
    }
    ...
    public double getPrice(Player player) {
        return (15 * Math.pow(1.15, getSum(player)));
    }

    @Override
    public double getSum(Player player) {
        return plugin.getUpgradesManager().get(player, "cursor");
    }

    @Override
    public double getMultiplier(Player player) {
        return (0.1 * plugin.getAchievementManager().getCombinedMultiplier(player, "cursor") + plugin.getAchievementManager().getCombinedTerm(player, "cursor")) * getSum(player);
    }
}
