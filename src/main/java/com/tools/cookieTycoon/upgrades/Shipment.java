...
public class Shipment extends Upgrade {

    public Shipment(CookieTycoon plugin) {
        super(plugin);
    }
    ...
    public String getName() {
        return ChatColor.BOLD + "Космическая доставка";
    }

    @Override
    public String getLore() {
        return ChatColor.YELLOW + "Доставляет свежее печенье с планеты печенья.";
    }
    ...
    public double getPrice(Player player) {
        return (51 * Math.pow(10, 8) * Math.pow(1.15, getSum(player))); // 5.1B = 51 * 10^8 * 1,15^N
    }

    @Override
    public double getSum(Player player) {
        return plugin.getUpgradesManager().get(player, "shipment");
    }

    @Override
    public double getMultiplier(Player player) {
        return (260000 * plugin.getAchievementManager().getCombinedMultiplier(player, "shipment") + plugin.getAchievementManager().getCombinedTerm(player, "shipment"))* getSum(player);
    }
}
